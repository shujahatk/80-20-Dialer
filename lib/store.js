  ...
  async approveUser(id) {
    if (isSupabaseConfigured()) {
      try {
        ...
        let query = getSupabaseClient().from('users').update({ approved: true, active: true });
        
        if (idStr.includes('@')) {
          query = query.eq('email', idStr.toLowerCase());
        } else if (isUuid) {
          query = query.or(`id.eq.${idStr},_id.eq.${idStr}`);
        } else {
          query = query.eq('_id', idStr);
        }

        const { data, error } = await query.select();
        if (error) console.error('[Supabase] approveUser error:', error.message);
        if (data && data.length > 0) {
          const { password, ...rest } = data[0];
          return { ...rest, _id: data[0]._id || data[0].id };
        }
      } catch (e) {
        console.error('[Supabase] approveUser exception:', e.message);
      }
    }
    if (isMongoConnected()) {
  ...
  async updateRole(id, role) {
    if (isSupabaseConfigured()) {
      try {
        const idStr = String(id).trim();
        const isUuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(idStr);
        let query = getSupabaseClient().from('users').update({ role });
