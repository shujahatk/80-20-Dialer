  ...
  async rejectUser(id) {
    if (isSupabaseConfigured()) {
      try {
        ...
        if (isEmail) {
          await client.from('users').delete().eq('email', idStr.toLowerCase());
        } else if (isUuid) {
          const { data } = await client.from('users').delete().eq('id', idStr).select();
          if (!data || data.length === 0) {
            await client.from('users').delete().eq('_id', idStr);
          }
        } else {
          await client.from('users').delete().eq('_id', idStr);
        }
