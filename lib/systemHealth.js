...
export async function getSystemHealth() {
  ...
  try {
    await checkDatabaseHealth(); services.database = 'connected';
  ...
  } catch { services.database = 'unavailable'; }
  ...
  return { status: healthy ? 'ok' : 'degraded', services, uptimeSeconds: Math.floor(process.uptime()), timestamp: new Date().toISOString() };
}
