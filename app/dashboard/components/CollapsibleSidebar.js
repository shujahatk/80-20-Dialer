...
export default function CollapsibleSidebar({ activeTab = '', setActiveTab = null, user = null, onlineCount = 0 }) {
  const router = useRouter();
  const [collapsed, setCollapsed] = useState(false);

  const mainNav = [
    { id: 'overview', label: 'Overview', icon: 'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6' },
    { id: 'analytics', label: 'Analytics & History', icon: 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z' },
    { id: 'pipeline', label: 'Visual Pipeline', icon: 'M9 17V7m0 10a2 2 0 01-2 2H5a2 2 0 01-2-2V7a2 2 0 012-2h2a2 2 0 012 2m0 10a2 2 0 002 2h2a2 2 0 002-2M9 7a2 2 0 012-2h2a2 2 0 012 2m0 10V7m0 10a2 2 0 002 2h2a2 2 0 002-2V7a2 2 0 00-2-2h-2a2 2 0 00-2 2' },
    { id: 'leads', label: 'Lead Management Hub', icon: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z' },
  ...
  return (
    ...
    >
      ...
      <div className="p-3 border-t border-white/5 flex items-center justify-between">
        ...
        >
          <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-indigo-500/30 to-cyan-500/30 flex items-center justify-center font-bold text-xs text-cyan-300 shrink-0 border border-cyan-500/20 shadow-sm" suppressHydrationWarning>
            {user?.name?.[0]?.toUpperCase() || 'A'}
          </div>
          {!collapsed && (
            <div className="truncate">
              <div className="text-xs font-bold text-slate-200 truncate" suppressHydrationWarning>{user?.name || 'Administrator'}</div>
              <div className="text-[10px] text-cyan-400 capitalize font-medium" suppressHydrationWarning>{user?.role || 'owner'} · Settings</div>
            </div>
          )}
        </div>
        {!collapsed && (
